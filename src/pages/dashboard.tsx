...
export function DashboardPage() {
  ...
  const monthlyTotal = useMemo(() => 
    expenses.filter(e => e.amount > 0).reduce((sum, e) => sum + e.amount, 0),
    [expenses]
  );
  
  const monthlyIncome = useMemo(() =>
    expenses.filter(e => e.amount < 0).reduce((sum, e) => sum + Math.abs(e.amount), 0),
    [expenses]
  );
          ...
          <div className="md:hidden">
            <div className="space-y-4">
              ...
              <div className="pt-3 border-t text-sm text-muted-foreground flex items-center gap-2">
                ...
                <span>{t('dashboard.tapForStats') || 'Tap per vedere statistiche'}</span>
              </div>
            </div>
          </div>
        ...
        <div className="hidden md:grid md:grid-cols-2 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500 delay-200">
          <Card className="hover:shadow-lg transition-all">
            <CardHeader>
              ...
              <CardDescription>{t('dashboard.topCategoriesDesc') || 'Dove spendi di più'}</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
          ...
          <Card className="hover:shadow-lg transition-all">
            <CardHeader>
              <div className="flex items-center gap-2">
                ...
                <CardTitle>{t('dashboard.overallStats') || 'Statistiche'}</CardTitle>
              </div>
              <CardDescription>{t('dashboard.overallStatsDesc') || 'Panoramica'}</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-6">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-muted-foreground">{t('dashboard.avgExpense') || 'Media Giornaliera'}</p>
        ...
        <CardContent>
          {expenses.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              ...
              <Button variant="link" onClick={() => navigate('/expense/new')} className="mt-2">
                  ...
                  >
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-muted-foreground">
                        {categoryObj ? `${categoryObj.icon} ${categoryObj.name}` : 'Unknown'}
                      </p>
                    </div>
                    <div className="text-right">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {format(new Date(expense.date), 'd MMM', { locale: dateLocale })}
                      </p>
                    </div>
                  </div>
                );
